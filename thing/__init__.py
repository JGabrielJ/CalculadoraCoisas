...
CONVERSION_OPTIONS = {
    'Ângulo': ('Grau', 'Radiano', 'Grado'), # OK
    'Área': ('Centímetro Quadrado (cm²)', 'Metro Quadrado (m²)', 'Quilômetro Quadrado (km²)', 'Hectare (ha)'), # OK
    'Comprimento': ('Milímetro (mm)', 'Centímetro (cm)', 'Metro (m)', 'Quilômetro (km)', 'Milha (mi)', 'Pé (ft)', 'Polegada (in)'), # OK
    'Contagem': ('Unidade', 'Dezena', 'Centena', 'Milhar', 'Milhão', 'Bilhão', 'Trilhão'), # OK
    'Dados': ('Bit (b)', 'Byte (B)', 'Kilobyte (kB)', 'Megabyte (MB)', 'Gigabyte (GB)', 'Terabyte (TB)'), # OK
    'Energia': ('Joule (J)', 'Caloria Alimentar (kcal)', 'Quilowatt-hora (kWh)'), # OK
    'Frequência': ('Hertz (Hz)', 'Quilo-hertz (kHz)', 'Mega-hertz (MHz)', 'Gigahertz (GHz)'), # OK
    'Moeda': ('Real (BRL)', 'Dólar (USD)', 'Euro (EUR)', 'Bitcoin (BTC)'), # OK
    'Numeração': ('Binário', 'Octal', 'Decimal', 'Hexadecimal'), # OK
    'Peso': ('Miligrama (mg)', 'Grama (g)', 'Quilograma (kg)', 'Tonelada (t)', 'Libra (lb)'), # OK
    'Potência': ('Watt (W)', 'Quilowatt (kW)', 'Cavalo-Vapor (hp)'), # OK
    'Pressão': ('Atmosfera (atm)', 'Milímetro de Mercúrio (mmHg)', 'Pascal (Pa)'), # OK
    'Temperatura': ('Celsius', 'Fahrenheit', 'Kelvin'), # OK
    'Tempo': ('Milissegundo', 'Segundo', 'Minuto', 'Hora', 'Dia', 'Semana', 'Mês', 'Ano', 'Década', 'Século', 'Milênio'), # OK
    'Velocidade': ('Centímetro por Segundo (cm/s)', 'Metro por Segundo (m/s)', 'Quilômetro por Hora (km/h)', 'Milha por Hora (mph)', 'Mach (Ma)'), # OK
    'Volume': ('Mililitro (mL)', 'Litro (L)', 'Metro Cúbico (m³)'), # OK
}
...
CONVERSION_FACTORS = {
    'Área': {
        ...
        'unidades': {
            'Centímetro Quadrado (cm²)': 0.0001, 'Metro Quadrado (m²)': 1.0,
            'Quilômetro Quadrado (km²)': 1_000_000.0, 'Hectare (ha)': 10_000.0,
        }
    },
    'Energia': {
        'base': 'Joule (J)',
        'unidades': {
            'Joule (J)': 1, 'Caloria Alimentar (kcal)': 4_184, 'Quilowatt-hora (kWh)': 3_600_000,
        }
    },
    'Frequência': {
        'base': 'Hertz (Hz)',
        'unidades': {
            'Hertz (Hz)': 1, 'Quilo-hertz (kHz)': 1_000,
            'Mega-hertz (MHz)': 1_000_000, 'Gigahertz (GHz)': 1_000_000_000,
        }
    },
    'Comprimento': {
        'base': 'Metro (m)',
        'unidades': {
            'Milímetro (mm)': 0.001, 'Centímetro (cm)': 0.01, 'Metro (m)': 1.0, 'Quilômetro (km)': 1_000.0,
            'Milha (mi)': 1609.34, 'Pé (ft)': 0.3048, 'Polegada (in)': 0.0254,
        }
    },
    'Contagem': {
        'base': 'Unidade',
        'unidades': {
            'Unidade': 1, 'Dezena': 10, 'Centena': 100, 'Milhar': 1_000,
    ...
    'Dados': {
        'base': 'Byte (B)',
        'unidades': {
            'Bit (b)': 0.125, 'Byte (B)': 1.0, 'Kilobyte (kB)': 1_000.0, 'Megabyte (MB)': 1_000_000.0,
            'Gigabyte (GB)': 1_000_000_000.0, 'Terabyte (TB)': 1_000_000_000_000.0,
        }
    },
    'Peso': {
        'base': 'Quilograma (kg)',
        'unidades': {
            'Miligrama (mg)': 0.000001, 'Grama (g)': 0.001, 'Quilograma (kg)': 1.0,
            'Tonelada (t)': 1_000.0, 'Libra (lb)': 0.453592,
        }
    },
    'Potência': {
        'base': 'Watt (W)',
        'unidades': {
            'Watt (W)': 1.0, 'Quilowatt (kW)': 1_000.0, 'Cavalo-Vapor (hp)': 745.7,
        }
    },
    'Pressão': {
        'base': 'Pascal (Pa)',
        'unidades': {
            'Atmosfera (atm)': 101_325.0, 'Pascal (Pa)': 1.0,
    ...
    'Velocidade': {
        ...
        'unidades': {
            'Centímetro por Segundo (cm/s)': 0.01, 'Metro por Segundo (m/s)': 1.0,
            'Quilômetro por Hora (km/h)': 0.277778, 'Milha por Hora (mph)': 0.44704,
    ...
    'Volume': {
        'base': 'Litro (L)',
        'unidades': {
            'Mililitro (mL)': 0.001, 'Litro (L)': 1.0, 'Metro Cúbico (m³)': 1_000.0,
        }
    },
}
...
def _convert_angle(value: float, from_unit: str, to_unit: str) -> float:
    ...
    degree = 0.0
    match from_unit:
        case 'Grau': degree = value
        case 'Radiano': degree = value * 180 / pi
        case 'Grado': degree = value * 180 / 200

    # Converte da base para a unidade de saída
    match to_unit:
        case 'Grau': return degree
        case 'Radiano': return degree * pi / 180
        case 'Grado': return degree * 200 / 180
...
def _convert_time(value: float, from_unit: str, to_unit: str) -> float:
    ...
    to_days_factors = {
        ...
        'Segundo': 1 / (60 * 60 * 24), 'Minuto': 1 / (60 * 24),
        'Hora': 1 / 24, 'Dia': 1, 'Semana': 7, 'Mês': DAYS_IN_YEAR / 12,
        'Ano': DAYS_IN_YEAR, 'Década': DAYS_IN_YEAR * 10,
        'Século': DAYS_IN_YEAR * 100, 'Milênio': DAYS_IN_YEAR * 1000,
    }
...
def _fetch_currency_rates() -> dict | None:
    ...
    if _currency_cache['rates'] and (now - _currency_cache['timestamp'] < _currency_cache['ttl']):
    ...
    try:
        ...
        if data.get("result") == "success":
            ...
            btc_response = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd", timeout=5)
            btc_response.raise_for_status(); btc_data = btc_response.json(); data['rates']['BTC'] = 1 / btc_data['bitcoin']['usd']
