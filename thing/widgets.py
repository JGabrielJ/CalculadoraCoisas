...
class DateSelector(ttk.Frame):
    def __init__(self, parent: tk.Widget, controller: Any, **kwargs):
        ...
        btn_cal = ttk.Button(self, text='▼', style='CDC.TButton', command=self.show_calendar)
        btn_cal.grid(row=0, column=1, sticky='e'); self._app_toplevel = self.controller
    ...
    def get_date(self) -> date | None:
        ...
        if date_str:
            try:
                py_date_pattern = self._date_pattern.replace('dd', '%d')\
    ...
    def show_calendar(self, event=None) -> None:
        ...
        cal = Calendar(top, **cal_kwargs); cal.pack(padx=10, pady=10)

        def set_date() -> None:
            """Define a data no seletor e fecha a janela."""
            self.data_var.set(cal.get_date()); top.destroy()

        btn_ok = ttk.Button(top, text='Confirmar', style='CDC.TButton',
                            command=set_date); btn_ok.pack(pady=5)
