...
class DateCalc(ttk.Frame):
    def __init__(self, parent: tk.Widget, controller: Any) -> None:
        ...
        ttk.Label(self, text='Cálculo de Datas', style='TTL.TLabel', anchor='center')\
        ...
        ttk.Radiobutton(mode_frame, text='Diferença entre Datas', variable=self.mode_var,
                        style='CDC.TRadiobutton', value='diff', command=self.__update_layout)\
                            .grid(row=0, column=0, padx=10, sticky='w')

        ttk.Radiobutton(mode_frame, text='Adicionar/Subtrair Data', variable=self.mode_var,
                        style='CDC.TRadiobutton', value='adsb', command=self.__update_layout)\
                            .grid(row=0, column=1, padx=10, sticky='e')

        # Frame 1: Diferença entre Datas
        self.diff_frame = ttk.LabelFrame(self, text=' Calcular a Diferença ',
                                         labelanchor='nw', padding=(5, 10))
        self.diff_frame.grid_columnconfigure(1, weight=1)

        # Widgets da Opção de Cálculo 1
        ttk.Label(self.diff_frame, text='Data Inicial:', style='PRG.TLabel')\
            .grid(row=0, column=0, padx=5, pady=5, sticky='e')

        self.start_date_entry = DateSelector(self.diff_frame, controller,
        ...
        ttk.Label(self.diff_frame, text='Data Final:', style='PRG.TLabel')\
            .grid(row=1, column=0, padx=5, pady=5, sticky='e')

        self.end_date_entry = DateSelector(self.diff_frame, controller,
                                           width=12, date_pattern='dd/mm/yyyy',
                                           background='darkblue', foreground='white',
                                           selectbackground='blue', selectforeground='white',
                                           headersbackground='gray', headersforeground='white')
        ...
        ttk.Button(self.diff_frame, text='Calcular Diferença', style='CDC.TButton', command=self.__calculate_difference)\
            .grid(row=2, column=0, columnspan=2, pady=10)

        self.diff_result_var = tk.StringVar(value='Diferença: ')
        ttk.Label(self.diff_frame, textvariable=self.diff_result_var, style='PRG.TLabel')\
            .grid(row=3, column=0, columnspan=2, pady=5)

        # Frame 2: Adicionar/Subtrair Data
        self.add_frame = ttk.LabelFrame(self, text=' Adicionar ou Subtrair de uma Data ',
                                        labelanchor='nw', padding=(5, 10))
        self.add_frame.grid_columnconfigure(1, weight=1)

        # Widgets da Opção de Cálculo 2
        ttk.Label(self.add_frame, text='Data de Partida:', style='PRG.TLabel')\
            .grid(row=0, column=0, padx=5, pady=5, sticky='e')

        self.base_date_entry = DateSelector(self.add_frame, controller,
                                            width=12, date_pattern='dd/mm/yyyy',
                                            background='darkblue', foreground='white',
                                            selectbackground='blue', selectforeground='white',
                                            headersbackground='gray', headersforeground='white')
        ...
        ttk.Label(time_units_frame, text='Dias:', style='PRG.TLabel').grid(row=0, column=0, sticky='e')
        self.days_spinbox = ttk.Spinbox(time_units_frame, from_=0, to=100, width=5)
        self.days_spinbox.grid(row=0, column=1, padx=(7, 15), sticky='w'); self.days_spinbox.set(0)

        ttk.Label(time_units_frame, text='Meses:', style='PRG.TLabel').grid(row=0, column=2, sticky='e')
        self.months_spinbox = ttk.Spinbox(time_units_frame, from_=0, to=100, width=5)
        self.months_spinbox.grid(row=0, column=3, padx=(7, 15), sticky='w'); self.months_spinbox.set(0)

        ttk.Label(time_units_frame, text='Anos:', style='PRG.TLabel').grid(row=0, column=4, sticky='e')
        self.years_spinbox = ttk.Spinbox(time_units_frame, from_=0, to=100, width=5)
        self.years_spinbox.grid(row=0, column=5, padx=(7, 15), sticky='w'); self.years_spinbox.set(0)
    ...
    def __calculate_difference(self) -> None:
        ...
        try:
            ...
            if start_date and end_date:
                ...
                self.diff_result_var.set('Erro! Ambas as datas devem ser selecionadas!!!')
