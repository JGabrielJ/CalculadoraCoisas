...
class Converter(ttk.Frame):
    def __init__(self, parent: tk.Widget, controller: Any) -> None:
        ...
        self.category_menu = ttk.Combobox(
            ...
            font=("Consolas", 10),
            values=list(CONVERSION_OPTIONS.keys()),
            state="readonly",
        )
        ...
        ttk.Label(self, text="De:", anchor="center", style="PRG.TLabel").grid(
            row=3, column=0, padx=8, sticky="nsew"
        )
        ...
        ttk.Label(self, text="Para:", anchor="center", style="PRG.TLabel").grid(
            row=5, column=0, padx=8, pady=(0, 20), sticky="nsew"
        )
    ...
    def __button_click(self, button: str) -> None:
        ...
        if button == "✓":
            ...
            result = convert(
                ...
                value=float(unformat_number(input_val)),
            )
    ...
    def __delete(self) -> None:
        ...
        if len(current) == 1:
            ...
            self.input_value_var.set(format_number(current[:-1]))
    ...
    def __append_digit(self, number: str) -> None:
        ...
        if len(current) < self.max_digits:
            if current == "0" and number != ".":
                self.input_value_var.set(format_number(number))
            else:
                # Evita adicionar múltiplos pontos decimais
                if number == "." and "." in current:
                    return
                self.input_value_var.set(format_number(current + number))
